...
\author{
A. Alpha,%\altaffilmark{1},
B. Beta%\altaffilmark{2}
}
...
(Table~\ref{tab:obs}).
Obsid 055100201 (PI: Motch) targeted the Be star HD 119682, but also captured
...
% Overview / summary
\paragraph{Fitting Summary}
The integrated remnant is well fit to a single temperature, non-equilibrium
ionization plasma with above-solar silicon and sulfur abundances (\texttt{vnei}
in XSPEC, after \citet{hamilton1983}).  \textbf{(is there a better reference?)}

Most emission is concentrated in 100\arcsec--300\arcsec, about half-way in from
the radio-inferred remnant edge.


We also fit spectra from $100\arcsec$ wide annuli centered on the remnant
to discern how thermal plasma emission varies with remnant radius.
...
The half-energy enclosed raiud

We further fit annuli spectra centered on the remnant

We further extract spectra from fit the remnant using
...
% TODO need to revisit this for thoroughness.
...
for each plasma.  % TODO provide error bars for the norms here!
...
\begin{table*}[!h]
    \centering
    \caption{Motch MOS1 BACKSCAL ratio has very little effect on fits
